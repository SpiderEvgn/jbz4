class Wechat::Zhizhu::Seat < ActiveRecord::Base
  include HTTParty
  ...
  def self.getSeat(cinemaId, hallId)
    # 2.2.3 座位
    client_key = ENV['JBZ4_ZHIZHU_CLIENT_KEY']
    private_key = ENV['JBZ4_ZHIZHU_PRIVATE_KEY']
    sign_value = Digest::MD5.hexdigest("#{cinemaId}#{hallId}#{client_key}#{private_key}")
    response = get("/seatList.html", query: { key: "#{client_key}", 
                                              cinemaId: "#{cinemaId}",
                                              hallId: "#{hallId}",
                                              sign: "#{sign_value}"
                                              })['seatList']
